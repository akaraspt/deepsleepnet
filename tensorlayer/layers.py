...
class Word2vecEmbeddingInputlayer(Layer):
    ...
    nce_b_init : NCE decoder biases initializer
        The initializer for tf.compat.v1.get_variable() of the nce decoder bias vector.
    nce_b_init_args : a dictionary
        The arguments for tf.compat.v1.get_variable() of the nce decoder bias vector.
    name : a string or None
        An optional name to attach to this layer.
...
class EmbeddingInputlayer(Layer):
    ...
    >>> y_ = tf.compat.v1.placeholder(tf.int32, shape=[batch_size, 1])
...
class DenseLayer(Layer):
    ...
    W_init_args : dictionary
        The arguments for the weights tf.compat.v1.get_variable.
    b_init_args : dictionary
        The arguments for the biases tf.compat.v1.get_variable.
    name : a string or None
        An optional name to attach to this layer.
...
class DeConv2dLayer(Layer):
    ...
    >>> batch_size = 64
    >>> inputs = tf.compat.v1.placeholder(tf.float32, [batch_size, 100], name='z_noise')
...
class AtrousConv2dLayer(Layer):
    ...
    W_init_args : dictionary. The arguments for the weights tf.compat.v1.get_variable().
    b_init_args : dictionary. The arguments for the biases tf.compat.v1.get_variable().
...
#
#               update_moving_mean = tf.compat.v1.assign(moving_mean,
#                                    moving_mean * decay + mean * (1 - decay))
#               update_moving_variance = tf.compat.v1.assign(moving_variance,
...
#         scope = name
#         try:
#             vscope = tf.compat.v1.variable_scope(scope, default_name=name, values=[self.inputs],)
#                                     #    reuse=reuse)
#         except Exception:
...
#             name = scope.name
#         # with tf.compat.v1.variable_scope(name) as vs:
#             beta = tf.compat.v1.get_variable('beta', shape=[input_shape[-1]],
#                                 initializer=beta_init,)
#                             #    initializer=tf.constant_initializer(beta),)
#                             #    trainable=trainable, )#restore=restore)
#             gamma = tf.compat.v1.get_variable('gamma', shape=[input_shape[-1]],
...
#                 inference.set_shape(input_shape)
#
#             variables = tf.compat.v1.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope.name)    # 2 params beta, gamma
#                 # variables = [beta, gamma, moving_mean, moving_variance]
#
#             # print(len(variables))
#             # for idx, v in enumerate(variables):
...
#             beta = tf.compat.v1.get_variable("beta", shape=params_shape,
#                         initializer=beta_init)
#             pop_mean = tf.compat.v1.get_variable("pop_mean", shape=params_shape,
#                         initializer=tf.zeros_initializer, trainable=False)
#             pop_var = tf.compat.v1.get_variable("pop_var", shape=params_shape,
...
#             # variables = tf.compat.v1.get_collection(TF_GRAPHKEYS_VARIABLES, scope=vs.name)  # 8 params in TF12 if zero_debias=True
#             variables = tf.compat.v1.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=vs.name)    # 2 params beta, gamma
#                 # variables = [beta, gamma, moving_mean, moving_variance]
#
#             # print(len(variables))
#             # for idx, v in enumerate(variables):
...
class PoolLayer(Layer):
    ...
    ``tf.nn.max_pool2d`` and ``tf.nn.avg_pool`` for 2D or
    ``tf.nn.max_pool2d3d`` and ``tf.nn.avg_pool3d`` for 3D.
    ...
    pool : a pooling function
        ...
        - class ``tf.nn.max_pool2d``
        - class ``tf.nn.avg_pool``
        - class ``tf.nn.max_pool2d3d``
...
class TimeDistributedLayer(Layer):
    ...
    >>> timestep = 20
    >>> input_dim = 100
    >>> x = tf.compat.v1.placeholder(dtype=tf.float32, shape=[batch_size, timestep,  input_dim], name="encode_seqs")
    >>> net = InputLayer(x, name='input')
...
class RNNLayer(Layer):
    ...
    - For CNN+LSTM
    >>> x = tf.compat.v1.placeholder(tf.float32, shape=[batch_size, image_size, image_size, 1])
    >>> network = tl.layers.InputLayer(x, name='input_layer')
    >>> network = tl.layers.Conv2dLayer(network,
    ...                         act = tf.nn.relu,
    ...                         shape = [5, 5, 1, 32],  # 32 features for each 5x5 patch
...
class DynamicRNNLayer(Layer):
    ...
    Examples
    --------
    >>> input_seqs = tf.compat.v1.placeholder(dtype=tf.int64, shape=[batch_size, None], name="input_seqs")
    >>> network = tl.layers.EmbeddingInputlayer(
    ...             inputs = input_seqs,
    ...             vocabulary_size = vocab_size,
    ...             embedding_size = embedding_size,
...
class Seq2Seq(Layer):
    ...
    >>> batch_size = 32
    >>> encode_seqs = tf.compat.v1.placeholder(dtype=tf.int64, shape=[batch_size, None], name="encode_seqs")
    >>> decode_seqs = tf.compat.v1.placeholder(dtype=tf.int64, shape=[batch_size, None], name="decode_seqs")
    >>> target_seqs = tf.compat.v1.placeholder(dtype=tf.int64, shape=[batch_size, None], name="target_seqs")
    >>> target_mask = tf.compat.v1.placeholder(dtype=tf.int64, shape=[batch_size, None], name="target_mask") # tl.prepro.sequences_get_mask()
    >>> with tf.compat.v1.variable_scope("model"):
    ...     # for chatbot, you can use the same embedding layer,
    ...     # for translation, you may want to use 2 seperated embedding layers
    >>>     with tf.compat.v1.variable_scope("embedding") as vs:
